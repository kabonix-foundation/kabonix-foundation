...
export async function handleAdminRoute({ parts, method, body, user, url, res, send, checkPermission, can }) {
  ...
  if (parts[2] === 'roles' && parts[3] && parts[4] === 'permissions' && method === 'PUT') {
    ...
    // Use a single dedicated client so BEGIN/DELETE/INSERT/COMMIT all run on
    // the same connection. pool.query('BEGIN') does not work — the pool may
    // hand out a different client for each statement.
    ...
    // Permissions changed for every user holding this role — drop the whole
    // cache so the change is visible on the next request, not in 30s.
  ...
  if (parts[2] === 'users' && parts[4] === 'roles' && method === 'PUT') {
    ...
    // The target user's cached permission set is now stale.
    invalidateUserPermissions(targetId);
  ...
  if (parts[2] === 'audit' && method === 'GET') {
    ...
    // `to` is a bare date (YYYY-MM-DD); interpret it as end-of-day so today's
    // events are included when today is picked as the end of the range.
