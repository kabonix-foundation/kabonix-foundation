...
// Where contact-form and registration notifications go during testing.
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // Creates the account inactive with approval_status='pending'. The user
    // cannot sign in until an administrator approves them via the portal.
    if (parts[1] === 'auth' && parts[2] === 'register' && req.method === 'POST') {
      ...
      // Best-effort ping to the admin inbox.
      await sendMail({
        to: CONTACT_INBOX,
    ...
    if (parts[1] === 'auth' && parts[2] === 'login' && req.method === 'POST') {
      ...
      // Password is confirmed, so we can safely give specific guidance about
      // account state without leaking whether an email exists.
